...
PipeClient::PipeClient(HANDLE a_pipeHandle, IPipeClient::MsgHandlerT&& a_msgHandler) :
    m_pipeHandle(a_pipeHandle), m_msgHandler(std::move(a_msgHandler)) {
...
void PipeClient::RunMsgHandler() {
	...
	while(!m_closed) {
		...
		auto error  = GetLastError();
		if(!result && error == ERROR_IO_PENDING) {
			if(GetOverlappedResult(m_pipeHandle, &overlapped, &bytesRead, TRUE) != 0) m_msgHandler(msg, bytesRead);
		}
	}
}
