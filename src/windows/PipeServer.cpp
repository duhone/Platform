...
namespace CR::Platform {
	class PipeServer : public IPipeServer {
	  ...
	  private:
		...
		OperationHandlerT m_operationHandlers[3] = {&PipeServer::OnConnection, &PipeServer::OnRead,
		                                            &PipeServer::OnWrite};
...
PipeServer::PipeServer(HANDLE a_pipeHandle, WriteFinishedT&& a_writeFinished, ReadFinishedT&& a_readFinished) :
	...
	m_iocpPort = OpenIOCPPort(m_pipeHandle,
	                          [this](auto a_overlapped, auto a_size) { this->OnOverlapped(a_overlapped, a_size); });
...
void PipeServer::WriteMsg(void* a_msg, size_t a_msgSize) {
	...
	auto error  = GetLastError();
	if(!result && error != ERROR_IO_PENDING) { OnWrite(op, a_msgSize); }
}

std::unique_ptr<IPipeServer> CR::Platform::CreatePipeServer(const char* a_name,
                                                            IPipeServer::WriteFinishedT a_writeFinished,
	...
	if(pipeHandle == INVALID_HANDLE_VALUE) return nullptr;
